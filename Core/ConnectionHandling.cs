...
namespace Raven.Core
{
    public class ConnectionHandling
    {
        ...
        private void manager_connectionEvent(ConnectionInformation connection)
        {
            ...
            RavenEnvironment.GetGame().GetClientManager().CreateAndStartClient(Convert.ToInt32(connection.getConnectionID()), connection);
        }
        ...
        private void CloseConnection(ConnectionInformation Connection)
        {
            ...
            {
                Connection.Dispose();
                RavenEnvironment.GetGame().GetClientManager().DisposeConnection(Convert.ToInt32( Connection.getConnectionID()));
            }
            catch (Exception e)
            {
