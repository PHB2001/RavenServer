...
namespace Raven.Communication.Packets.Incoming.Groups
{
    class GetForumsListDataEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                case 2:
                    var Forums = RavenEnvironment.GetGame().GetGroupForumManager().GetForumsByUserId(Session.GetHabbo().Id);
                ...
                case 0:
                    ...
                    using (var adap = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        adap.AddParameter("now", (int)RavenEnvironment.GetUnixTimestamp());
                    ...
                    {
                        GroupForum forum;
                        if (RavenEnvironment.GetGame().GetGroupForumManager().TryGetForum(Convert.ToInt32(Row["id"]), out forum))
                            forums.Add(forum);
                    }
                    break;
