...
namespace Raven.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Room.GetRoomItemHandler().GetWallAndFloor.Count() > RavenStaticGameSettings.RoomFurnitureLimit)
            {
                Session.SendNotification("no se puede tener mas de " + RavenStaticGameSettings.RoomFurnitureLimit + " furnis en una sala!");
                return;
            }
            ...
            if (!Item.IsWallItem)
            {
                ...
                if (Room.GetRoomItemHandler().SetFloorItem_ball(Session, RoomItem, X, Y, Rotation, true, false, true))
                {
                    Session.GetHabbo().GetInventoryComponent().RemoveItem(ItemId);
