...
namespace Raven.Communication.Packets.Incoming.Catalog
{
    class BuyTargettedOfferMessage : IPacketEvent
    {
...

        {
            ...
            var offer = RavenEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer;
            ...
            using (IQueryAdapter dbQuery = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                        ...
                        foreach (var product in offer.Products)
                        {
                            ...
                            switch (product.ItemType)
                            {
                                ...
                                case "item":
                                    {
                                        ItemData item = null;
                                        if (!RavenEnvironment.GetGame().GetItemManager().GetItem(int.Parse(product.Item), out item)) return;
                                        if (item == null) return;
            ...
            #region RE-OPEN TARGETED BOX
                    TargetedOffers TargetedOffer2 = RavenEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer;
                            ...
                            {
                                Session.SendMessage(RavenEnvironment.GetGame().GetTargetedOffersManager().TargetedOffer.Serialize());
                            }
                        }
                    }
            #endregion
