...
namespace Raven.Communication.Packets.Incoming.Catalog
{
    class CheckGnomeNameEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                RavenEnvironment.GetGame().GetClientManager().ManagerAlert(RoomNotificationComposer.SendBubble("estaff", "El usuario " + Session.GetHabbo().Username + " se ha autentificado correctamente", ""));
            ...
            else
            {
                ...
                RavenEnvironment.GetGame().GetClientManager().ManagerAlert(new MOTDNotificationComposer("ATENCION: El Staff " + Session.GetHabbo().Username + " ha fallado al autentificar su identidad."));
