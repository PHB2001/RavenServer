...
namespace Raven.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogEvent : IPacketEvent
    {
        ...
        {
            if (RavenEnvironment.GetDBConfig().DBData["catalogue_enabled"] != "1")
            {
            ...
            if (baseItem != null)
            {
                ...
                if (baseItem.InteractionType == InteractionType.tag)
                {
                    ...
                    string character;
                    ExtraData = RavenEnvironment.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(ExtraData, out character) ? "" : ExtraData;

                    if (string.IsNullOrEmpty(ExtraData))
                    {
                        Session.SendNotification("Lo sentimos, pero la plabara " + character + " no es una palabra permitida dentro de los filtros de "+RavenEnvironment.GetDBConfig().DBData["hotel.name"]+", por ello no puedes ponerlo");
                        return;
                    }
            ...
            if (Item.IsLimited)
            {
                ...
                RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_LTDPurchased", 1);
            ...
            if (Item.PredesignedId > 0 && RavenEnvironment.GetGame().GetCatalog().GetPredesignedRooms().predesignedRoom.ContainsKey((uint)Item.PredesignedId))
            {
                #region SELECT ROOM AND CREATE NEW
                var predesigned = RavenEnvironment.GetGame().GetCatalog().GetPredesignedRooms().predesignedRoom[(uint)Item.PredesignedId];
                var decoration = predesigned.RoomDecoration;

                var createRoom = RavenEnvironment.GetGame().GetRoomManager().CreateRoom(Session, Session.GetHabbo().Username + "'s room", "Una Sala pre-decorada :)", predesigned.RoomModel, 1, 25, 1);
                ...
                var newRoom = RavenEnvironment.GetGame().GetRoomManager().LoadRoom(createRoom.Id);
                #endregion
                ...
                RavenEnvironment.GetGame().GetRoomManager().LoadRoom(newRoom.Id).GetRoomItemHandler().LoadFurniture();
            ...
            switch (Item.Data.InteractionType)
            {
                ...
                case InteractionType.GUILD_FORUM:
                    ...
                    Group gp;
                    if (!RavenEnvironment.GetGame().GetGroupManager().TryGetGroup(GroupId, out gp))
                        break;

                    if (gp.HasForum)
                        break;

                    var forum = RavenEnvironment.GetGame().GetGroupForumManager().CreateGroupForum(gp);
                ...
                case InteractionType.LANDSCAPE:
                    ...
                    try
                    {
                        if (string.IsNullOrEmpty(ExtraData))
                            ...
                            Number = Double.Parse(ExtraData, RavenEnvironment.CultureInfo);
                    }
                    catch (Exception e)
                    {
