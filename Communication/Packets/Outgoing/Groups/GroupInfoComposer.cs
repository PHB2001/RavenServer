...
namespace Raven.Communication.Packets.Outgoing.Groups
{
    class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
            ...
            base.WriteString((RavenEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "No room found.." : RavenEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
            ...
            base.WriteString(RavenEnvironment.GetUsernameById(Group.CreatorId));
