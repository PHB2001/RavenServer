    ...
    class UsersComposer : ServerPacket
    {
        ...
        private void WriteUser(RoomUser User)
        {
            if (!User.IsPet && !User.IsBot)
            {
                ...
                if (Habbo != null)
                {
                    if (Habbo.GetStats() != null)
                    {
                        ...
                        {
                            if (!RavenEnvironment.GetGame().GetGroupManager().TryGetGroup(Habbo.GetStats().FavouriteGroupId, out Group))
                                Group = null;
                        }
                    }
                }
            ...
            else if (User.IsBot)
            {
                ...
                base.WriteString(RavenEnvironment.GetUsernameById(User.BotData.ownerID)); // Owner name
