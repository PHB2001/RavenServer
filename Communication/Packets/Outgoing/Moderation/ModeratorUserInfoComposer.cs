    ...
    class ModeratorUserInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
           ...
           base.WriteString(User != null ? Convert.ToString(User["look"]) : "Unknown");
            base.WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((RavenEnvironment.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) / 60)) : 0);
            base.WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((RavenEnvironment.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60)) : 0);
            base.WriteBoolean(User != null ? RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) != null : false);
