...
namespace Raven.Communication.Packets.Outgoing.Messenger
{
    class BuddyListComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.BuddyListMessageComposer)
        {
            ...
            if (Player.TeamRank >= 1) friendCount++;
            if (Player.Rank >= 5) friendCount++;

            base.WriteInteger(1);
            base.WriteInteger(0);
            var groups = RavenEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Player.Id).Where(c => c.HasChat).ToList();
            ...
            if (Player.TeamRank >= 1)
            {
                base.WriteInteger(int.MinValue + 1);
                base.WriteString("Teams");
                base.WriteInteger(1);
                base.WriteBoolean(true);
                base.WriteBoolean(false);
