...
namespace Raven.HabboHotel.Cache.Process
{
    sealed class ProcessComponent
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.Cache.Process.ProcessComponent");
        ...
        public void Run(object State)
        {
            try
            {
                ...
                // BEGIN CODE
                List<UserCache> CacheList = RavenEnvironment.GetGame().GetCacheManager().GetUserCache().ToList();
                if (CacheList.Count > 0)
                {
                    foreach (UserCache Cache in CacheList)
                    {
                        try
                        {
                            ...
                            if (Cache.isExpired())
                                RavenEnvironment.GetGame().GetCacheManager().TryRemoveUser(Cache.Id, out Temp);
                ...
                CacheList = null;
                    ...
                    List<Habbo> CachedUsers = RavenEnvironment.GetUsersCached().ToList();
                        ...
                        foreach (Habbo Data in CachedUsers)
                        {
                            try
                            {
                                ...
                                if (Data.CacheExpired())
                                    RavenEnvironment.RemoveFromCache(Data.Id, out Temp);
