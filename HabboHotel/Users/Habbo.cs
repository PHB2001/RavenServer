...
using Raven.HabboHotel.Rooms.Chat.Commands;
using Raven.HabboHotel.Users.Permissions;
using Raven.HabboHotel.Subscriptions;
using Raven.HabboHotel.Club;
using System.Linq;
using Raven.Communication.Packets.Outgoing.Rooms.Notifications;
using Raven.Communication.Packets.Outgoing.LandingView;
using Raven.HabboHotel.Users.Polls;
using Raven.HabboHotel.Catalog;
using Raven.HabboHotel.Items;
using Raven.HabboHotel.Helpers;

namespace Raven.HabboHotel.Users
{
    public class Habbo
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.Users");
        ...
        public Habbo(int Id, string Username, int Rank, int TeamRank, string Motto, string Look, string Gender, int Credits, int ActivityPoints, int HomeRoom,
         ...
         byte guidelevel, byte publicistalevel, byte builder, byte croupier, bool Nux, byte TargetedBuy, string NameColor, string Tag, string TagColor, byte NameChange, string PinClient)
        {
            ...
            this._motto = Motto;
            this._look = RavenEnvironment.GetGame().GetAntiMutant().RunLook(Look);
            this._gender = Gender.ToLower();
            this._footballLook = RavenEnvironment.FilterFigure(Look.ToLower());
            ...
            if (this._tradingLockExpiry > 0 && RavenEnvironment.GetUnixTimestamp() > this.TradingLockExpiry)
            {
            ...
            this._sessionStart = RavenEnvironment.GetUnixTimestamp();
            this._messengerSpamCount = 0;
            this._messengerSpamTime = 0;
            this._creditsTickUpdate = RavenStaticGameSettings.UserCreditsUpdateTimer;
                ...
                try
                {
                    ...
                    if (Convert.ToString(StatRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        if (this._permissions.HasRight("mod_tool"))
                            DailyRespects = 20;
                        else if (RavenEnvironment.GetGame().GetSubscriptionManager().TryGetSubscriptionData(VIPRank, out SubData))
            ...
            Group G = null;
            if (!RavenEnvironment.GetGame().GetGroupManager().TryGetGroup(this._habboStats.FavouriteGroupId, out G))
        ...
        public int TeamRank
        {
            get { return this._teamrank; }
            set { this._teamrank = value; }
        }
        ...
        {
            int level = this._Talents.Values.Select(current => RavenEnvironment.GetGame().GetTalentManager().GetTalent(current.TalentId).Level).Concat(new[] { 1 }).Max();
            return level;
        }
        ...
        public Room CurrentRoom
        {
            get
            {
                ...
                Room _room = null;
                if (RavenEnvironment.GetGame().GetRoomManager().TryGetRoom(CurrentRoomId, out _room))
                    return _room;
        ...
        public void OnDisconnect()
        {
            ...
            if(_onHelperDuty)
            {
                GameClient Session = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(this.Id);
                HelperToolsManager.RemoveHelper(Session);
            }
            
            RavenEnvironment.GetGame().GetClientManager().UnregisterClient(Id, Username);
            ...
            if (!this._habboSaved) // GUARDADO DE USER
            {
                ...
                using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.runFastQuery("UPDATE `users` SET `online` = '0', `last_online` = '" + RavenEnvironment.GetUnixTimestamp() + "', `activity_points` = '" + this.Duckets + "', `credits` = '" + this.Credits + "',  `vip_points` = '" + this.Diamonds + "' ,  `bonus_points` = '" + this._BonusPoints + "', `home_room` = '" + this.HomeRoom + "', `gotw_points` = '" + this.GOTWPoints + "', `user_points` = '" + this.UserPoints + "', `time_muted` = '" + this.TimeMuted + "',`friend_bar_state` = '" + FriendBarStateUtility.GetInt(this._friendbarState) + "' WHERE id = '" + Id + "' LIMIT 1;UPDATE `user_stats` SET `roomvisits` = '" + this._habboStats.RoomVisits + "', `onlineTime` = '" + (RavenEnvironment.GetUnixTimestamp() - this.SessionStart + this._habboStats.OnlineTime) + "', `respect` = '" + this._habboStats.Respect + "', `respectGiven` = '" + this._habboStats.RespectGiven + "', `giftsGiven` = '" + this._habboStats.GiftsGiven + "', `giftsReceived` = '" + this._habboStats.GiftsReceived + "', `dailyRespectPoints` = '" + this._habboStats.DailyRespectPoints + "', `dailyPetRespectPoints` = '" + this._habboStats.DailyPetRespectPoints + "', `AchievementScore` = '" + this._habboStats.AchievementPoints + "', `quest_id` = '" + this._habboStats.QuestID + "', `quest_progress` = '" + this._habboStats.QuestProgress + "', `groupid` = '" + this._habboStats.FavouriteGroupId + "',`forum_posts` = '" + this._habboStats.ForumPosts + "',`PurchaseUsersConcurrent` = '" + this._habboStats.PurchaseUsersConcurrent + "' WHERE `id` = '" + this.Id + "' LIMIT 1;");
        ...
        public void CheckBonusTimer()
        {
            try
            {
                ...
                if (this._bonusTickUpdate <= 0)
                {
                    ...
                    this.BonusUpdateTick = RavenStaticGameSettings.BonusRareUpdateTimer;
                }
            }
            catch { }
        }
        public void CheckCreditsTimer()
        {
            try
            {
                ...
                {
                    int CreditUpdate = RavenStaticGameSettings.UserCreditsUpdateAmount;
                    int DucketUpdate = RavenStaticGameSettings.UserPixelsUpdateAmount;
                    int VipDucketUpdate = RavenStaticGameSettings.UserVipPixelsUpdateAmount;
                    int DiamondUpdate = RavenStaticGameSettings.UserDiamondsUpdateAmount;
                    int VipDiamondUpdate = RavenStaticGameSettings.UserVipDiamondsUpdateAmount;

                    this._credits += CreditUpdate;
                    this._diamonds += DiamondUpdate;
                    this._diamonds += VipDiamondUpdate;
                    ...
                    if (this._client.GetHabbo().Rank == 2 && this._client.GetHabbo().VIPRank == 1)
                    {
                        ...
                        this._client.SendMessage(new HabboActivityPointNotificationComposer(this.Diamonds, VipDiamondUpdate, 5));
                    }
                    else
                    {
                        this._client.SendMessage(new CreditBalanceComposer(this._credits));
                    ...
                    this.CreditsUpdateTick = RavenStaticGameSettings.UserCreditsUpdateTimer;
                }
            }
            catch { }
        }
        ...
        public GameClient GetClient()
        {
            ...
            return RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(Id);
        }
        ...
        {
            this.LastNameChange = RavenEnvironment.GetUnixTimestamp();
        ...
        public void PrepareRoom(int Id, string Password)
        {
            ...
            {
                Room OldRoom = null;
                if (!RavenEnvironment.GetGame().GetRoomManager().TryGetRoom(this.GetClient().GetHabbo().CurrentRoomId, out OldRoom))
                    return;
            ...
            Room Room = RavenEnvironment.GetGame().GetRoomManager().LoadRoom(Id);
            if (Room == null)
            {
                this.GetClient().SendMessage(new CloseConnectionComposer(this.GetClient()));
                return;
        ...
        {
            if (!RavenEnvironment.GetGame().GetCalendarManager().CampaignEnable())
                return;

            calendarGift = new bool[RavenEnvironment.GetGame().GetCalendarManager().GetTotalDays()];
            ...
            using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("name", RavenEnvironment.GetGame().GetCalendarManager().GetCampaignName());
        ...
        public bool EnterRoom(Room Room)
        {
            ...
            using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ('" + this.GetClient().GetHabbo().Id + "','" + this.GetClient().GetHabbo().CurrentRoomId + "','" + RavenEnvironment.GetUnixTimestamp() + "','0','" + DateTime.Now.Hour + "','" + DateTime.Now.Minute + "');");// +
            }
            ...
            if (Room.OwnerId != this.Id)
            {
                ...
                RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(this.GetClient(), "ACH_RoomEntry", 1);            
            }
            return true;
        }
    }
