    ...
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = RavenEnvironment.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        {
            GameClient friend = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (friend == null || friend.GetHabbo() == null)
            {
                ...
                newFriend = new MessengerBuddy(friendID, Convert.ToString(dRow["username"]), Convert.ToString(dRow["look"]), Convert.ToString(dRow["motto"]), Convert.ToInt32(dRow["last_online"]),
                    RavenEnvironment.EnumToBool(dRow["hide_online"].ToString()), RavenEnvironment.EnumToBool(dRow["hide_inroom"].ToString()));
            }
            else
            {
        ...
        public bool RequestBuddy(string UserQuery)
        {
            ...
            GameClient client = RavenEnvironment.GetGame().GetClientManager().GetClientByUsername(UserQuery);
            if (client == null)
            {
                DataRow Row = null;
                using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
                ...
                hasFQDisabled = RavenEnvironment.EnumToBool(Row["block_newfriends"].ToString());
            }
            else
            {
            ...
            RavenEnvironment.GetGame().GetQuestManager().ProgressUserQuest(GetClient(), QuestType.ADD_FRIENDS);

            GameClient ToUser = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (ToUser == null || ToUser.GetHabbo() == null)
                return true;

            MessengerRequest Request = new MessengerRequest(ToId, _userId, RavenEnvironment.GetGame().GetClientManager().GetNameById(_userId));

            ToUser.GetHabbo().GetMessenger().OnNewRequest(_userId);

            UserCache ThisUser = RavenEnvironment.GetGame().GetCacheManager().GenerateUser(_userId);
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, RavenEnvironment.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            #region Custom Chats
            var Group = RavenEnvironment.GetGame().GetGroupManager().GetGroupsForUser(GetClient().GetHabbo().Id).Where(c => c.HasChat).ToList();
            foreach (var gp in Group)
            {
                if (ToId == int.MinValue + gp.Id) // int.MaxValue
                {
                    //RavenEnvironment.GetGame().GetClientManager().SendMessaget(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                    RavenEnvironment.GetGame().GetClientManager().GroupChatAlert(new FuckingConsoleMessageComposer(int.MinValue + gp.Id, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), gp, GetClient().GetHabbo().Id);
                    return;
                }
            ...
            {
                RavenEnvironment.GetGame().GetClientManager().StaffAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            else if (GetClient().GetHabbo().TeamRank >= 1 && ToId == (int.MinValue + 1))
            {
                RavenEnvironment.GetGame().GetClientManager().TeamAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            #endregion
            ...
            {
                GetClient().GetHabbo().MessengerSpamTime = RavenEnvironment.GetUnixTimestamp() + 60;
            ...
            else if (GetClient().GetHabbo().MessengerSpamTime > RavenEnvironment.GetUnixTimestamp())
            {
                double Time = GetClient().GetHabbo().MessengerSpamTime - RavenEnvironment.GetUnixTimestamp();
            ...
            GameClient Client = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = RavenEnvironment.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }
