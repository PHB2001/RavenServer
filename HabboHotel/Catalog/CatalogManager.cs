...
using Raven.HabboHotel.Catalog.Pets;
using Raven.HabboHotel.Catalog.Vouchers;
using Raven.HabboHotel.Catalog.Marketplace;
using Raven.HabboHotel.Catalog.Clothing;

using Raven.Database.Interfaces;
using Raven.HabboHotel.Catalog.PredesignedRooms;

namespace Raven.HabboHotel.Catalog
{
    public class CatalogManager
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.Catalog.CatalogManager");
                ...
                if (CatalogueItems != null)
                {
                    foreach (DataRow Row in CatalogueItems.Rows)
                    {
                        ...
                        this._items[PageId].Add(Convert.ToInt32(Row["id"]), new CatalogItem(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["item_id"]),
                            ...
                            Convert.ToInt32(Row["amount"]), Convert.ToInt32(Row["limited_sells"]), Convert.ToInt32(Row["limited_stack"]), RavenEnvironment.EnumToBool(Row["offer_active"].ToString()),
