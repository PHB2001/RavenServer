...
namespace Raven.HabboHotel.Rooms
{
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(GameClient Session)
        {
            ...
            if (Session.GetHabbo().TeamRank == 3 && _room.Id == 1296 && !Session.GetHabbo().DisableForcedEffects)
                Session.GetHabbo().Effects().ApplyEffect(597);
            if (Session.GetHabbo().TeamRank == 2 && _room.Id == 1296 && !Session.GetHabbo().DisableForcedEffects)
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.runFastQuery("UPDATE user_roomvisits SET exit_timestamp = '" + RavenEnvironment.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
        ...
        private void RemoveRoomUser(RoomUser user)
        {
            if (user.SetStep)
            {
                ...
                _room.GetGameMap().RemoveUserFromMap(user, new Point(user.SetX, user.SetY));
                //MithServer.GetGame().GetClientManager().SendMessage(new BroadcastMessageAlertComposer("BUG ENCONTRADO" + "\r\n" + "- " + user.GetClient().GetHabbo().Username));
            }
            else
            ...
            try
            {
                ...
                foreach (RoomUser User in GetUserList().ToList())
                {
                    ...
                    if (User.SetStep)
                    {
                        ...
                        if (this._room.GetGameMap().IsValidStep2(User, new Vector2D(User.X, User.Y), new Vector2D(User.SetX, User.SetY), (User.GoalX == User.SetX && User.GoalY == User.SetY), User.AllowOverride))
                        {
                            if (!User.RidingHorse)
                    ...
                    if (User.IsWalking && !User.Freezed)
                    {
                        ...
                        if (InvalidStep || (User.PathStep >= User.Path.Count) || (User.GoalX == User.X && User.GoalY == User.Y)) //No path found, or reached goal (: (User.PathStep >= User.Path.Count) <<<
                        {
                ...
                {
                    GameClient client = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (RavenEnvironment.GetUnixTimestamp() > RavenEnvironment.GetUnixTimestamp() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
                                    ...
                                    if (!RavenEnvironment.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room))
                                        return;
                        ...
                        case InteractionType.PINATA:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 158)
                                    {
                                        ...
                                        if (givenHits == 1) RavenEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);

                                        #region Achievements
                                        RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataWhacker", 1);
                                        RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataBreaker", 1);
                                        #endregion
                        ...
                        case InteractionType.PLANT_SEED:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 192)
                                    {
                                        ...
                                        if (givenHits > 5) RavenEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);
                                        {
                                            givenHits = 0;
                                            Item.ExtraData = givenHits.ToString();
                                            Item.UpdateState();
