...
namespace Raven.HabboHotel.Rooms
{
    public class RoomUser
    {
        ...
        public string GetUsername()
        {
            ...
            if (GetClient() != null)
            {
                if (GetClient().GetHabbo() != null)
                {
                    ...
                    return RavenEnvironment.GetUsernameById(HabboId);
        ...
        public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            if (Shout)
                Packet = new ShoutComposer(VirtualId, ColouredMessage, RavenEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            else
                Packet = new ChatComposer(VirtualId, ColouredMessage, RavenEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
        ...
        public void SendNameColourPacket()
        {
            ...
            else
            {
                if (GetClient().GetHabbo()._tagcolor == "rainbow")
                    Username = "<font color='" + RavenEnvironment.RainbowT() + "'>[" + GetClient().GetHabbo()._tag + "]</font> <font size =\"12\" color=\"#" + GetClient().GetHabbo()._nameColor + "\">" + GetUsername() + "</font>";
                else
        ...
        public GameClient GetClient()
        {
            ...
            if (mClient == null)
                mClient = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(HabboId);
            return mClient;
        }
        ...
        {
            if (mRoom == null)
                if (RavenEnvironment.GetGame().GetRoomManager().TryGetRoom(RoomId, out mRoom))
                    return mRoom;
