...
namespace Raven.HabboHotel.Rooms.Chat.Commands.Moderator
{
    class GuideAlertCommand : IChatCommand
    {
        ...
        {
            if (Session.GetHabbo().TeamRank != 1 || Session.GetHabbo().TeamRank != 2 || Session.GetHabbo().TeamRank != 3)
            {
            ...
            RavenEnvironment.GetGame().GetClientManager().TeamAlert(new MOTDNotificationComposer("[GUIDE]\r[" + dtDateTime + "]\r\r" + Message + "\r\r - " + Session.GetHabbo().Username + " [" + Session.GetHabbo().TeamRank + "]"));
            return;
        }
    }
}