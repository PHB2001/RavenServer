...
using Raven.HabboHotel.Items.Data.Toner;
using Raven.HabboHotel.Rooms.Games.Freeze;
...
using Raven.HabboHotel.Rooms.Games.Banzai;
...
using Raven.HabboHotel.Rooms.Games.Football;
...
namespace Raven.HabboHotel.Rooms
{
    public class Room : RoomData
    {
        ...
        public void AssignNewOwner(Room ForSale, RoomUser Buyer, RoomUser Seller)
        {
                ...
                if (ForSale.Group != null)
                {
                    ...
                    if (GroupID > 0)
                    {
                        ...
                        foreach (int UserID in ForSale.Group.GetAllMembers)
                        {
                            ...
                            GameClient UserClient = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(UserID);
                            if (UserClient == null)
                                continue;
                    ...
                    RavenEnvironment.GetGame().GetGroupManager().DeleteGroup(ForSale.Group.Id);
            ...
            Seller.GetClient().GetHabbo().Diamonds += ForSale.ForSaleAmount;
            Seller.GetClient().SendMessage(new HabboActivityPointNotificationComposer(Buyer.GetClient().GetHabbo().Diamonds, ForSale.ForSaleAmount));
            Buyer.GetClient().GetHabbo().Diamonds -= ForSale.ForSaleAmount;
            Buyer.GetClient().SendMessage(new HabboActivityPointNotificationComposer(Buyer.GetClient().GetHabbo().Diamonds, ForSale.ForSaleAmount));
            ...
            foreach (RoomUser User in UsersReturn)
            {
                ...
                User.GetClient().SendNotification("La sala donde usted acaba de estar fue comprada por " + Buyer.GetClient().GetHabbo().Username + " en total " + ForSale.ForSaleAmount + " diamantes!");
            }
        }
        ...
        public bool OnUserShoot(RoomUser User, Item Ball)
        {
            ...
            foreach (Item item in this.GetRoomItemHandler().GetFurniObjects(Ball.GetX, Ball.GetY).ToList())
            {
                if (item.GetBaseItem().ItemName.StartsWith("fball_goal_"))
                {
                    ...
                    User.UnIdle();
                    User.DanceId = 0;
                    RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FootballGoalScored", 1);
        ...
        public void ProcessRoom()
        {
            ...
            try
            {
                ...
                {
                    RavenEnvironment.GetGame().GetRoomManager().UnloadRoom(this);
                    return;
                }
