...
namespace Raven.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            mStaticModel = RavenEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            if (mStaticModel == null)
            {
                RavenEnvironment.GetGame().GetRoomManager().LoadModel(room.ModelName);
                mStaticModel = RavenEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            }
        ...
        public void UpdateUserMovementReverse(Point oldCoord, Point newCoord, RoomUser user)
        {
            RemoveUserFromMap(user, newCoord);
            AddUserToMap(user, oldCoord);
        }
        ...
        public bool NextSquareHasUser(int x, int y, RoomUser User)
        {
            ...
            if (SquareHasUsers(newx, newy)) // aqui iba el 3
            {
        ...
        public void NextSquareHasUserMoveDiagonalPoint(int x, int y, RoomUser User)
        {
            int newx = x;
            int newy = y;
            if (User.RotBody == 1)
            {
                if (SquareHasUsers(newx, newy + 1))
                {
                    User.MoveTo(newx, newy + 1);
                }
            }

            if (User.RotBody == 3)
            {
                if (SquareHasUsers(newx - 1, newy))
                    User.MoveTo(newx - 1, newy);
            }
        ...
        public Point getRandomWalkableSquare()
        {
            ...
            int RandomNumber = RavenEnvironment.GetRandomNumber(0, walkableSquares.Count);
            int i = 0;
        ...
        public bool IsValidWalk(RoomUser User, Vector2D From, Vector2D To, bool Override)
        {
            ...
            if (Items.Count > 0)
            {
                ...
                if (HasGroupGate)
                {
                    ...
                    {
                        Group Group = null;
                        if (!RavenEnvironment.GetGame().GetGroupManager().TryGetGroup(I.GroupId, out Group))
                            return false;
            ...
            if (mCoordinatedItems.ContainsKey(coord))
            {
                ...
                foreach (Item item in itemsFromSquare)
                {
                    if (item.GetBaseItem().InteractionType == InteractionType.RENTABLE_SPACE)
                        ...
                        if (RavenEnvironment.GetGame().GetRentableSpaceManager().GetRentableSpaceItem(item.Id, out _rentableSpace))
                        {
                            ...
                            if (_rentableSpace.OwnerId == Session.GetHabbo().Id && RavenEnvironment.GetUnixTimestamp() < _rentableSpace.ExpireStamp)
                            {
