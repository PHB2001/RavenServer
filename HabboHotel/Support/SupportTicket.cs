...
namespace Raven.HabboHotel.Support
{
    ...
    public class SupportTicket
    {
        ...
        public SupportTicket(int Id, int Score, int Type, int SenderId, int ReportedId, String Message, int RoomId, String RoomName, Double Timestamp, List<string> ReportedChats)
        {
            ...
            SenderName = RavenEnvironment.GetGame().GetClientManager().GetNameById(SenderId);
            ReportedName = RavenEnvironment.GetGame().GetClientManager().GetNameById(ReportedId);
            ModName = RavenEnvironment.GetGame().GetClientManager().GetNameById(ModeratorId);
        }
        ...
        public void Pick(int pModeratorId, Boolean UpdateInDb)
        {
            ...
            ModName = RavenEnvironment.GetHabboById(pModeratorId).Username;
            if (UpdateInDb)
            {
                using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.RunQuery("UPDATE moderation_tickets SET status = 'picked', moderator_id = " + pModeratorId + ", timestamp = '" + RavenEnvironment.GetUnixTimestamp() + "' WHERE id = " + Id + "");
                }
            }
        }
