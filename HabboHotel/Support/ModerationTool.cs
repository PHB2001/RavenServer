...
//namespace Raven.HabboHotel.Support
//{   
//    /// <summary>
...
//                {
//                    dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'0','','" + RavenEnvironment.GetUnixTimestamp() + "')");
...
//                Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, 0, "", RavenEnvironment.GetUnixTimestamp(), Messages);
...
//            RoomData Data = RavenEnvironment.GetGame().GetRoomManager().GenerateRoomData(Session.GetHabbo().CurrentRoomId);

//            using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
//            {
//                dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'" + Data.Id + "',@name,'" + RavenEnvironment.GetUnixTimestamp() + "')");
//                dbClient.AddParameter("message", Message);
...
//            Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, Data.Id, Data.Name, RavenEnvironment.GetUnixTimestamp(), Messages);
//            Tickets.Add(Ticket);
//            SendTicketToModerators(Ticket);
//        }
...
//            GameClient Client = RavenEnvironment.GetGame().GetClientManager().GetClientByUserID(Ticket.SenderId);
...
//        {
//            RavenEnvironment.GetGame().GetClientManager().SendMessage(new ModeratorSupportTicketComposer(Ticket), "mod_tool");
//            RavenEnvironment.GetGame().GetClientManager().StaffAlert(RoomNotificationComposer.SendBubble("tickets", "Hay una nueva petición de ayuda, revísala cuanto antes.", ""));
