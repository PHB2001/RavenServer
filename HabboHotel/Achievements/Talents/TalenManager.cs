...
namespace Raven.HabboHotel.Achievements
{
    /// <summary>
    ///     Class TalentManager.
    /// </summary>
    public class TalentManager
    {
        ...
        public void CompleteUserTalent(GameClient session, Talent talent)
        {
            ...
            {
                ItemData item = null;
                if (!RavenEnvironment.GetGame().GetItemManager().GetItem(talent.PrizeBaseItem, out item))
                    return;
            ...
            using (IQueryAdapter queryReactor = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
            ...
            if (talent.Type == "citizenship")
            {
                ...
                {
                    case 3:
                        RavenEnvironment.GetGame().GetAchievementManager().ProgressAchievement(session, "ACH_Citizenship", 1);
                        break;
                    case 4:
