...
namespace Raven.HabboHotel.GameClients
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.GameClients.GameClientManager");
        ...
        public void TeamAlert(ServerPacket Message, int Exclude = 0)
        {
            foreach (GameClient client in this.GetClients.ToList())
            {
                if (client == null || client.GetHabbo() == null)
                ...
                if (client.GetHabbo().TeamRank < 1 || client.GetHabbo().Id == Exclude)
                    continue;
        ...
        public void SupportMessage(ServerPacket Message, int Exclude = 0)
        {
            foreach (GameClient client in this.GetClients.ToList())
            {
                ...
                if (client.GetHabbo().TeamRank == 3 || client.GetHabbo().Id == Exclude)
                    continue;
