...
namespace Raven.HabboHotel.Items.Data.Moodlight
{
    public class MoodlightData
    {
        ...
        public MoodlightData(int ItemId)
        {
            ...
            Enabled = RavenEnvironment.EnumToBool(Row["enabled"].ToString());
        ...
        public void UpdatePreset(int Preset, string Color, int Intensity, bool BgOnly, bool Hax = false)
        {
            ...
            using (IQueryAdapter dbClient = RavenEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("UPDATE room_items_moodlight SET preset_" + Pr + " = '@color," + Intensity + "," + RavenEnvironment.BoolToEnum(BgOnly) + "' WHERE item_id = '" + ItemId + "' LIMIT 1");
        ...
        public static MoodlightPreset GeneratePreset(string Data)
        {
            ...
            return new MoodlightPreset(Bits[0], int.Parse(Bits[1]), RavenEnvironment.EnumToBool(Bits[2]));
        }
