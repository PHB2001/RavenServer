...
namespace Raven.HabboHotel.Items.RentableSpaces
{
    public class RentableSpaceManager
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.Items.RentableSpaces");
                ...
                if (table != null)
                {
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            if (ownerid > 0)
                            {
                                Habbo owner = RavenEnvironment.GetHabboById(ownerid);
                                if (owner != null)
