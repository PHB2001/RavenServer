...
namespace Raven.HabboHotel.Items
{
    public class ItemDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Raven.HabboHotel.Items.ItemDataManager");
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = RavenEnvironment.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = RavenEnvironment.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = RavenEnvironment.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = RavenEnvironment.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = RavenEnvironment.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = RavenEnvironment.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = RavenEnvironment.EnumToBool(Row["is_rare"].ToString());
                            int ClothingId = Convert.ToInt32(Row["clothing_id"]);
                            bool ExtraRot = RavenEnvironment.EnumToBool(Row["extra_rot"].ToString());
